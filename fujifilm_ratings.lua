...
if not df.check_if_bin_exists("exiftool-dne") then
	print('Did not find exiftool-dne')
end
...
local function detect_rating(event, image)
	...
	if string.len(jpeg_result) > 0 then
		...
		dt.print_error("Using JPEG Rating: " .. tostring(jpeg_result))
		return
	end
	...
	if string.len(raf_result) > 0 then
		...
		dt.print_error("Using RAF Rating: " .. tostring(raf_result))
	end
end

dt.register_event("post-import-image", detect_rating)

dt.print_error("fujifilm_ratings loaded.")
